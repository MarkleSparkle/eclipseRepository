...
public class BlackJack {
	...
	static Queue <Card> player = new LinkedList <Card>();
	...
	public static void main(String[] args) {
		...
		do{
			...
			Ess.output("Chips: "+balance);
			Ess.lines(15);
			bet=bets.nextInt();
			Deck.setPot(bet);
			balance-=bet;
			Ess.output("You now have "+balance+" chips left.");
			Ess.output("The pot now has "+Deck.pot+" chips.");


			do{/*PLAYER'S TURN*/
				Ess.output("Would you like to hit or pass? (h)");
				input = sc.nextLine();
			...
			while(playerBusted==false&&dealerPlays==true){/*START DEALER'S TURN*/
				do{
					Deck.displayGameboard(player, computer, 0);
					if(Deck.getTotal(computer)<=17) computer.add(Deck.addCard(deck));//if they hit, add a card to the player's hand
						...
						if(Deck.getTotal(computer)>21){
							...
							Ess.DEBUG(DEBUGGABLE,"Counting Numbers");
							if(betHappenings==-1){
								bet=0;
								Ess.output("Dealer takes your loss. Chip balance: "+balance);
							}
